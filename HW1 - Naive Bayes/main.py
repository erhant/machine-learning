...
labelToGender = {1: "Female", 2: "Male"} # 1 is for Woman, 2 for Man, in total there are 180 Men 20 Woman, for both training and test.
...
class1_stddev = np.sqrt((
        ...
        - 2 * np.sum(np.dot(X_train_c1, np.dot(class1_means[:,np.newaxis], np.ones((1, len(class1_means)))) * np.identity(len(class1_means))), axis = 0) 
        + (len(y_train_c1) * (class1_means * class1_means)))
    /len(y_train_c1))
class2_stddev = np.sqrt((
        np.sum(X_train_c2 * X_train_c2, axis = 0) 
        - 2 * np.sum(np.dot(X_train_c2, np.dot(class2_means[:,np.newaxis], np.ones((1, len(class2_means)))) * np.identity(len(class2_means))), axis = 0) 
...
for i in range(len(y_train)):
    y_train_pred[i] = np.argmax(np.array([bayes(X_train[i], class1_means, class1_stddev, class1_prior), bayes(X_train[i], class2_means, class2_stddev, class2_prior)]))

for i in range(len(y_test)):
    y_test_pred[i] = np.argmax(np.array([bayes(X_test[i], class1_means, class1_stddev, class1_prior), bayes(X_test[i], class2_means, class2_stddev, class2_prior)]))
